document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm.addEventListener('submit', (event) => {
        ...
        fetch('http://localhost/PW_archivos_PHP/Tienda_Online_VG2_COMPLETO_MGZ/login.php', {
            method: 'POST',
            headers: {
     ...
     paypal.Buttons({
         ...
         onApprove: (data, actions) => {
             return actions.order.capture().then(details => {
                 ...
                 // Llamar al archivo PHP para actualizar el stock
                 const productosComprados = carrito.map(producto => ({
                     product_id: producto.id,
                     cantidad_comprada: 1 // Ajusta esto según la cantidad comprada
                 }));
